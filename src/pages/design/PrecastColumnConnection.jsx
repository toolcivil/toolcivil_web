...
const PrecastColumnConnection = () => {
  const { language } = useLanguage();
  const t = translations.precastColumn;
  ...
  const calculateConnection = (e) => {
    e.preventDefault();
    const { columnHeight, columnWidth, concreteGrade, steelGrade, cover, axialForce, bendingMoment, boltGrade, boltDiameter, boltCount } = formData;
    
    if (columnHeight && columnWidth && cover && axialForce && bendingMoment && boltDiameter && boltCount) {
      // Convert inputs to numbers
      const h = parseFloat(columnHeight);
      const b = parseFloat(columnWidth);
      const c = parseFloat(cover);
      const N = parseFloat(axialForce);
      ...
      // Calculate axial capacity (simplified)
      const Nc = 0.85 * parseFloat(concreteGrade) * b * h;
      
      // Calculate moment capacity (simplified)
      const Mc = 0.9 * parseFloat(steelGrade) * b * d_eff * d_eff / 4;
      
      // Calculate bolt capacity (simplified)
      const Nb = 0.6 * parseFloat(boltGrade) * Math.PI * d * d / 4 * n;
      
      // Calculate connection efficiency
      const efficiency = Math.min(Nc/N, Mc/M, Nb/N) * 100;
      
      setResult({
        axialCapacity: Nc.toFixed(2),
        momentCapacity: Mc.toFixed(2),
        boltCapacity: Nb.toFixed(2),
          ...
          {result && (
            <div className="mt-8 p-6 bg-gray-50 rounded-lg">
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-sm text-gray-600">{t.axialCapacity[language]}</p>
                  <p className="text-2xl font-bold text-blue-600">
                    {result.axialCapacity} kN
                  </p>
                </div>
                <div>
...
export default PrecastColumnConnection; 