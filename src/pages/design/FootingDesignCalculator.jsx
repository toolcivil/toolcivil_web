...
const FootingDesignCalculator = () => {
  const { language } = useLanguage();
  const t = translations.footingDesign;
  ...
  const calculateFootingDesign = (e) => {
    e.preventDefault();
    const { columnWidth, columnDepth, axialLoad, momentX, momentY, soilBearingCapacity, footingDepth } = formData;
    
    if (columnWidth && columnDepth && axialLoad && soilBearingCapacity && footingDepth) {
      ...
      const h = parseFloat(footingDepth);
      
      // Calculate required footing area
      const requiredArea = P / qa;
      
      // Calculate footing dimensions (assuming square footing)
      const footingSize = Math.sqrt(requiredArea);
      ...
      const punchingShearForce = P * (1 - Math.pow(cw + d, 2) / Math.pow(footingSize, 2));
      ...
      const minAs = 0.002 * footingSize * footingSize;
      ...
      const spacing = (footingSize * barArea) / finalAs;
      
      setResult({
        footingSize: footingSize.toFixed(2),
        footingArea: requiredArea.toFixed(2),
        ...
        bearingPressure: (P / (footingSize * footingSize)).toFixed(2)
      });
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg p-8">
          <form onSubmit={calculateFootingDesign} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {t.footingDepth[language]}
                </label>
                <input
                  type="number"
                  name="footingDepth"
                  value={formData.footingDepth}
                  onChange={handleInputChange}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  placeholder="0.00"
                  step="0.1"
          ...
          {result && (
            <div className="mt-8 p-6 bg-gray-50 rounded-lg">
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-sm text-gray-600">{t.footingSize[language]}</p>
                  <p className="text-2xl font-bold text-blue-600">
                    {result.footingSize} m
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">{t.footingArea[language]}</p>
                  <p className="text-2xl font-bold text-blue-600">
                    {result.footingArea} m²
                  </p>
                </div>
                <div>
...
export default FootingDesignCalculator; 