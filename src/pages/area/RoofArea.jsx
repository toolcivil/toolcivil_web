...
const RoofArea = () => {
  ...
  const calculateArea = (e) => {
    e.preventDefault();
    const { length, width, pitch } = dimensions;
    if (length && width && pitch) {
      // Convert pitch to radians
      const pitchRad = (parseFloat(pitch) * Math.PI) / 180;
      // Calculate roof area using pitch angle
      const roofArea = (parseFloat(length) * parseFloat(width)) / Math.cos(pitchRad);
      setResult({
        ...
        <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg p-8">
          ...
          {result && (
            <div className="mt-8 p-6 bg-gray-50 rounded-lg">
              ...
              <div>
                <p className="text-sm text-gray-600">{t.roofArea[language]}</p>
                <p className="text-2xl font-bold text-blue-600">{result.roofArea} m²</p>
              </div>
            </div>
          )}
        </div>
